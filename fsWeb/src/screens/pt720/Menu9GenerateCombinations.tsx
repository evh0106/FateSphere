...
import {
  deleteGeneratedFilesPt720,
  generateFatePt720,
  generateMyCombinationsPt720,
  getFateFileContentPt720,
  getGeneratedFileContentPt720,
...
export default function Menu9GenerateCombinations({ runTask, setLastResponse, setMessage }: MenuProps) {
  ...
  async function loadGeneratedFiles() {
    const data = await getGeneratedFilesPt720();
  ...
  function handleDeleteFiles() {
    ...
    runTask(async () => {
      const result = await deleteGeneratedFilesPt720(selectedFiles);
  ...
  function handleFileClick(fileName: string) {
    runTask(async () => {
      const data = await getGeneratedFileContentPt720(fileName);
  ...
  function handleFateFileClick(fileName: string) {
    runTask(async () => {
      const data = await getFateFileContentPt720(fileName);
  ...
  function submitGenerateFate() {
    ...
    runTask(async () => {
      const res = await generateFatePt720(selectedFileForFate, count);
          ...
          <tbody>
            {generatedRows.map((combo, index) => (
              <tr key={`${combo.join("-")}-${index}`}>
                <td>{index + 1}</td>
                <td style={{ color: "#525252", fontWeight: "bold" }}>{combo[0]}</td>
                {combo.slice(1).map((num, i) => (
                  <td key={i} style={{ color: getNumberColor(i), fontWeight: "bold" }}>
                    {String(num)}
                  </td>
                ))}
              </tr>
        ...
        <button
          ...
          onClick={() =>
            runTask(async () => {
              ...
              const data = await generateMyCombinationsPt720(count);
              setGeneratedRows(data.combinations);
            ...
            {fileRows.length === 0 ? (
              <tr>
                <td colSpan={4} style={{ textAlign: "center", color: "var(--fg-muted)", padding: "1.5rem" }}>
                  No generated files in pt720/db/gn.
                </td>
              </tr>
            ) : (
      ...
      {savedFile && (
        <p style={{ fontSize: "0.85rem", color: "var(--fg-muted)", marginTop: "0.5rem" }}>
          Saved to: <code style={{ color: "var(--fg-default)" }}>
            {savedFile.startsWith("fate_") ? `pt720/db/fate/${savedFile}` : `pt720/db/gn/${savedFile}`}
          </code>
        </p>
      )}
                ...
                <table className="data-table" style={{ margin: 0, border: "none" }}>
                  ...
                  <tbody>
                    {fateFileRows.length === 0 ? (
                      <tr>
                        <td colSpan={8} style={{ textAlign: "center", color: "var(--fg-muted)", padding: "1rem" }}>
